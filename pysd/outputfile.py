...
class CoordFile(OutputFile):
    def __init__(self, fname=None):
        super().__init__(fname, dtype=[('i', 'i4'), ('x', 'f8'), ('y', 'f8'), ('z', 'f8'), ('site_num', 'i4'), ('atom_type', 'i4')])
...
class StructFile(OutputFile):
    def __init__(self, fname=None):
        super().__init__(fname, dtype=[('iatom', 'i4'), ('jatom', 'i4'), ('itype', 'i4'), ('jtype', 'i4'), ('r_x', 'f8'), ('r_y', 'f8'), ('r_z', 'f8'), ('J', 'f8'), ('r_abs', 'f8')])

#Iter           <M>_x           <M>_y           <M>_z             <M>        M_{stdv}
class AveragesFile(OutputFile):
    def __init__(self, fname=None):
        super().__init__(fname, dtype=[('iter', 'i4'), ('M_x', 'f8'), ('M_y', 'f8'), ('M_z', 'f8'), ('M', 'f8'), ('M_stdv', 'f8')])

#Iter                 Tot                 Exc                 Ani                  DM                  PD               BiqDM                  BQ                 Dip              Zeeman                 LSF                Chir                Ring                  SA
class EnergyFile(OutputFile):
    def __init__(self, fname=None):
        super().__init__(fname, dtype=[('iter', 'i4'), ('tot', 'f8'), ('exc', 'f8'), ('ani', 'f8'), ('dm', 'f8'), ('pd', 'f8'), ('biqdm', 'f8'), ('bq', 'f8'), ('dip', 'f8'), ('zeeman', 'f8'), ('lsf', 'f8'), ('chir', 'f8'), ('ring', 'f8'), ('sa', 'f8')])

#Iter             <M>           <M^2>           <M^4>      U_{Binder}            \chi        C_v(tot)             <E>       <E_{exc}>       <E_{lsf}>
class CumuFile(OutputFile):
    def __init__(self, fname=None):
        super().__init__(fname, dtype=[('iter', 'i4'), ('M', 'f8'), ('M2', 'f8'), ('M4', 'f8'), ('U_Binder', 'f8'), ('chi', 'f8'), ('C_v', 'f8'), ('E', 'f8'), ('E_exc', 'f8'), ('E_lsf', 'f8')])

#iter   ens   iatom           |Mom|             M_x             M_y             M_z
class MomentsFile(OutputFile):
    def __init__(self, fname=None):
        super().__init__(fname, dtype=[('iter', 'i4'), ('ens', 'i4'), ('iatom', 'i4'), ('M_abs', 'f8'), ('M_x', 'f8'), ('M_y', 'f8'), ('M_z', 'f8')])

    # return numpy array in shape (ens, iter, iatom)
    def moments(self):
        iters_ens = []
        iters = np.split(self.data, np.unique(self.data['iter'], return_index=True)[1][1:])
        for iter in iters:
            iters_ens.append(np.split(iter, np.unique(iter['ens'], return_index=True)[1][1:]))
